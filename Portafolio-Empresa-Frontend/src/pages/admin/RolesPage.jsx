...
export default function RolesPage() {
  ...
  const save = async () => {
    if (form.id) await roleService.update(form.id, form);
  ...
  const remove = async (row) => {
    ...
    await roleService.destroy(row.id);
    await load();
  };

  return (
    <AdminLayout>
      <CrudTable title="Roles" columns={[{ key: 'name', label: 'Nombre' }]} data={data} loading={loading}
        onAdd={openAdd} onEdit={openEdit} onDelete={remove} />

      {modal && (
        <Modal title={form.id ? 'Editar Rol' : 'Nuevo Rol'} onClose={closeModal}>
          <div className="mb-4">
